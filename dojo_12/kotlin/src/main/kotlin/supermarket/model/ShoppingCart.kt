    ...
    internal fun handleOffers(receipt: Receipt, offers: Map<Product, Offer>, catalog: SupermarketCatalog) {
        for (p in productQuantities().keys) {
            ...
            if (offers.containsKey(p)) {
                ...
                when (offer.offerTypeNew) {
                    is SpecialOfferTypeNew.ThreeForTwo -> {
                        val x = 3
                    ...
                    is SpecialOfferTypeNew.TwoForAmount -> {
                        var x = 2
                        if (quantityAsInt >= 2) {
                    ...
                    is SpecialOfferTypeNew.FiveForAmount -> {
                        if (quantityAsInt >= 5) {
                            var x = 5
                            val numberOfXs = quantityAsInt / x
                            val discountTotal =
                    ...
                    is SpecialOfferTypeNew.PercentDiscount -> {
                        var x = 1
                        val numberOfXs = quantityAsInt / x
                        if (offer.offerType === SpecialOfferType.PercentDiscount) {
                            val percentage = offer.offerTypeNew.percent;
                            discount =
                                Discount(p, percentage.toString() + "% off", quantity * unitPrice * percentage / 100.0)
                        }
                    }
                }
